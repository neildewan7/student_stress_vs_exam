...
const translations = {
  ...
  ja: {
    ...
    overview: {
      ...
      description2:
        "この研究に取り組む中で、試験パフォーマンスには最適なストレスレベルが存在すると予想しています。明らかに、ストレスが高す��ると、試験に適切に集中できず、パフォーマンスが低下します。しかし、全くストレスがない状態でも、脳が適切に集中できず、体に正しい信号を送るための試験の「脅威」を認識できないと考えています。そのため、体を訓練して最適なパフォーマンスを発揮できる中間的なストレスレベルが存在すると予想しています。",
      featured: "注目の学生の軌跡",
    },
    stories: {
      title: "学生の軌跡物語",
      intro:
        "個々の学生の学期を通じた軌跡を追跡します。各物語は、ストレスパターンが学業成績と個人の回復力とどのように相関するかを明らかにします。",
      comeback: {
        title: "カムバック物語",
    ...
    visualization: {
      ...
      hrChart: {
        title: "時間経過による心拍数",
        description:
          "この線グラフは、試験中の学生の平均心拍数を示しています。性別グループでフィルタリングして、男女学生の傾向を観察できます。時間軸は3時間の試験時間をカバーし、ホバーすると分レベルのHR詳細が表示されます。設計選択：線グラフは、試験前の急上昇や中間点後の平坦化などのパターンを特定するための時間的傾向を示すのに効果的であるため選択されました。",
        student: "学生：",
        exam: "試験：",
        xAxisLabel: "試験時間（分）",
        yAxisLabel: "心拍数（BPM）",
    ...
    patterns: {
      ...
      quiz: {
        ...
        intro:
          "いくつかの質問に答えて、あなたが最も示しやすいストレスパターンを発見してください：",
    ...
    insights: {
      ...
      realityCheck: {
        title: "現実チェック",
        description:
          "ストレスレベルと試験結果の間にはほとんど相関がありません。つまり、残念ながら試験で先んじる簡単な方法はありません。良い成績を取りたければ、一生懸命勉強するしかありません！",
      },
    },
    methods: {
      title: "データセットと方法論",
      description:
        "我々は、実際の大学試験中に収集された心拍数（HR）と皮膚電気活動（EDA）の連続記録を含む、PhysioNetから公開さ��ているウェアラブル試験ストレスデータセットを使用しました。",
      ...
      tools:
        "すべての前処理はPandasとNumPyを使用してPythonで行われました。可視化はインタラクティブ性と動的探索をサポートするためにD3.jsで開発されました。",
    },
    limitations: {
      title: "制限と考慮事項",
      list: [
    ...
    team: {
      title: "チームについて",
      description:
        "我々はデータストーリーテリン��に情熱を持つ学生のグループです。このプロジェクトは、最終的な可視化コース課題の一部として共同で構築されました。",
    },
  },
};
