...
const translations = {
  ...
  ja: {
    header: {
      ...
      authors:
        "著者：Jessica Jiaxin、Neil Dewan���Alex Evans、Seiichi Nakahira",
    },
    nav: {
      overview: "概要",
      stories: "学生の物語",
    ...
    overview: {
      ...
      description2:
        "この研究に取り組む中で、試験パフォーマンスには最適なストレスレベルが存在すると予想しています。明らかに、ストレスが高すぎると、試験に適切に集中できず、パフォーマンスが低下します。しかし、全くストレスがない状態でも、脳が適切に集中��きず、体に正しい信号を送るための試験の「脅威」を認識できないと考えています。そのため、体を訓練して最適なパフォーマンスを発揮できる中間的なストレスレベルが存在すると予想しています。",
      featured: "注目の学生の軌跡",
    },
    stories: {
      title: "学生の軌跡物語",
      ...
      comeback: {
        ...
        analysis:
          "この学生の軌跡は、ストレス管理の重要性を示しています。試験ごとにストレスレベルが低下するにつれて、パフォーマンスが劇的に向上しました。��末試験では、最高のパフォーマンスと相関するほぼ最適なストレスレベルを示しています。",
        insights: [
          "ストレスが段階的に減少：高→中→低",
    ...
    patterns: {
      ...
      quiz: {
        ...
        question1: "試験中、通常いつ最もストレ��を感じますか？",
        q1Options: {
          early: "最初から",
          middle: "途中で",
          late: "時間がなくなる終盤近く",
    ...
    insights: {
      ...
      heartRate: {
        title: "心拍数パターン",
        description:
          "心拍数は通常��試験の終盤近くでスパイクし、これ������間的プレッシャーや最後の努力を反映している可能性があります。一部の学生は、すべての試験で一貫したパターンを示します。",
      },
      stressVariability: {
        title: "ストレス変動性",
        description:
