...
const translations = {
  ...
  ja: {
    ...
    overview: {
      ...
      description2:
        "この研究に取り組む中で、試験パフォーマンスには最適なストレスレベルが存在すると予想しています。明らかに、ストレスが高すぎると、試験に適切に集中できず、パフォーマンスが低下します。しかし、全くストレスがない状態でも、脳が適切に集中できず、体に正しい信号を送るための��験の「脅威」を認識できないと考えています。そのため、体を訓練して最適なパフォーマンスを発揮できる中間的なストレスレベルが存在すると予想しています。",
      featured: "注目の学生の軌跡",
    },
    stories: {
      title: "学生の軌跡物語",
