...
const translations = {
  ...
  ja: {
    ...
    overview: {
      ...
      description2:
        "この研究に取り組む中で、試験パフォーマンスには最適なストレスレベルが存在すると予想しています。明らかに、ストレスが高すぎると、試験に適切に集中できず、パフォーマンスが低下します。しかし、全くストレスがない状態でも、脳が適切に集中できず、体に正しい信号を送るための試験の「脅威」を認識できないと考えています。そのため、体を訓練し��最適なパフォーマンスを発揮できる中間的なストレスレベルが存在すると予想しています。",
      featured: "注目の学生の軌跡",
    },
    stories: {
      title: "学生の軌跡物語",
      ...
      comeback: {
        ...
        analysis:
          "この学生の軌跡は、ストレス管理の重要性を示しています。試験ごとにストレスレベルが低下するにつれて、パフォーマンスが劇的に向上しまし��。期末試験では、最高のパフォーマンスと相関するほぼ最適なストレスレベルを示しています。",
        insights: [
          "ストレスが段階的に減少：高→中→低",
    ...
    visualization: {
      ...
      hrChart: {
        title: "時間経過による心拍数",
        description:
          "この線グラフは、試験中の学生の平均心拍数を示しています。性別グループでフィルタリングして、男女学生の傾向を観察できます。時間軸は3時間の試験時間をカバーし、ホバーすると分レベルのHR詳細が表示されます。設計選択：線グラフは、試験前の急上昇や中間点後の平坦化な��のパターンを特定するための時間的傾向を示すのに効果的であるため選択されました。",
        student: "学生：",
        exam: "試験：",
        xAxisLabel: "試験時間（分）",
        yAxisLabel: "心拍数（BPM）",
    ...
    patterns: {
      ...
      insights: {
        ...
        temporal: "時間的特徴",
        temporalDesc:
          "最も強力な識��要因は、ストレスのピークが試験中のいつ発生するかであり、強度だけではありません。",
