...
const translations = {
  ...
  ja: {
    header: {
      title: "ストレス信号：生理学が学業プレッシャーを反映��る方法",
      subtitle:
    ...
    stories: {
      ...
      comeback: {
        ...
        description:
          "学生S03は、初期の苦闘が最終的な結果を決定するものではないことを示しています。困難な最初の中間試験から始まり、より良いストレス管理技術を開発し、それが著しい改善につながりま���た。",
        analysis:
      ...
      steady: {
        ...
        description:
          "学生S01は着実で一貫した改善の典型例です。彼���のアプローチは、体系的な準備と一貫したストレスレベルが段階的だが信頼できる学業成長につながることを示しています。",
        analysis:
          "この学生は一貫性の力を示しています。劇的な変動ではなく、各試験サイクルを通じてパフォーマンスを徐々に向上させながら、安定したストレスレベ���を維持しました。",
        insights: [
          "一貫して中程度のストレスレベルを維持",
          "着実な改善を示した：78% → 82% → 91%",
          "一貫した準備の価値を実証",
      ...
      struggle: {
        ...
        analysis:
          "このケーススタディ��、管理されていないストレスがどのように負のフィードバックループを作り出すかを示しています。中間試験2でのストレスの劇的な急上昇は最悪のパフォーマンスと対応し、期末試験では部分的な回復がありましたが、高いストレスレベルが持続しました。",
        insights: [
    ...
    visualization: {
      ...
      hrChart: {
        title: "時間経過による心拍数",
        description:
          "この線グラフは、試験中の学生の平均心拍数を示しています。性別グループでフィルタリングして、男女���生の傾向を観察できます。時間軸は3時間の試験時間をカバーし、ホバーすると分レベルのHR詳細が表示されます。設計選択：線グラフは、試験前の急上昇や中間点後の平坦化などのパターンを特定するための時間的傾向を示すのに効果的であるため選択されました。",
        student: "学生：",
        exam: "試験：",
        xAxisLabel: "試験時間（分）",
        yAxisLabel: "心拍数（BPM）",
      ...
      scoreChart: {
        title: "信号強度対試験スコア",
        description:
          "この散布図は、学生の平均生理学的信号レベル（EDAやHRなど）と試験での成績を相関させています。中間試験と期末試験のデータを切り替えることができます。設計選択：ストレス指標と結果の間の可能な傾向を調査するために���布図を使用し、生理学的反応の高まりがより高いまたはより低い学業成績に関連するかどうかの探索を促進しています。",
        footnote:
    ...
    patterns: {
      ...
      quiz: {
        ...
        intro:
          "いくつかの質問に答えて、���なたが最も示し���すいストレスパターンを発見してください：",
    ...
    insights: {
      ...
      heartRate: {
        title: "心拍数パターン",
        description:
          "心拍数は通常、試験の終盤近くでスパイクし、これ���時間的プレッシャーや最後の努力を反映している可能性があります。一部の学生は、すべての試験で一貫したパターンを示します。",
      },
      stressVariability: {
        title: "ストレス変動性",
        description:
    ...
    methods: {
      title: "データセットと方法論",
      description:
        "我々は、実際の大学試験中に収集された心拍数（HR）と皮膚電気活動（EDA）の連続記録を含む、PhysioNetから公開��れているウェアラブル試験ストレスデータセットを使用しました。",
